...
```
As you can see there is no need for registering the `ParameterValidatorProxy<>` manually. You can create parameterized aspects as well:
```csharp
...
- You can enable *Strict DI* (disabled by default) in `Injector.config.json` which will instruct the injector to throw if a captive dependency can be found.
## MS preferred DI
...
- You should add +1 using (`Solti.Utils.DI.Interfaces`) in files where you use the general interfaces (e.g. `IServiceContainer`, `IInjector`, `AspectAttribute`, etc).
