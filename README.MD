...
- **Service recipe**: This is the most common way to file a service.
  ...
  - A service may request other services via the constructor parameters:
    ...
    - Lazy initialization also possible:
      ...
      ```
    - Starting from version 1.3.0, you can mark the requested dependency as optional. In this case the system won't throw if the dependency can not be found:
      ```csharp 
      public class MyService: IMyService
      {
      ...
      ```   
- **Lazy recipe**: Similar to the service recipe except that the implementation is unknown in build time (e.g. the service implementation is placed in a module we want to load on service request). So instead of using the implementation itself we pass its assembly and type name:
  ```csharp
  container.Lazy<IMyService>("Absolute.Path.To.Assembly", "Implementation.Fullname", Lifetime.Transient);
  ```
  Remarks:
  - The containing assembly will be loaded on the first request.
  - You may also register generic lazy services by the same way.
  - You can use your own type resolver by implementing the `ITypeResolver` interface.
...
- You may also register more service with the same interface by naming them:
  ...
  ```csharp
  using Solti.Utils.DI.Annotations;  
  ...
...
- Bulked decoration also possible:
  ```csharp
  using Solti.Utils.DI;
  using Solti.Utils.DI.Annotations;
  using Solti.Utils.Proxy;
  ...
  {
    Container
      .Setup(Assembly.GetExecutingAssembly()) // see "auto-config"
      .Service<IMyOtherService, MyOtherService>();
...
- You can setup transactions using proxies as well:
  ...
  ...
  Container
    .Factory<IDbConnection>(i => ..., Lifetime.Scoped)
...
After annotating the desired types you just have to simply call the `container.Setup(sourceAssembly)` (extension) method which instructs the system to seek and register all the annotated services from the `sourceAssembly`.
...
Sometimes the service being requested is contextual (for instance resolving the `HttpRequest` that triggered the current session). In this case you can derive a new container for the session:
```csharp
using Solti.Utils.DI;
...
// Create a temp container for the session
using (IServiceContainer tempContainer = Container.CreateChild())
{
  // We assume that there was no service registered with IHttpRequest interface
  tempContainer.Instance<IHttpRequest>(Request);

  // Disposing the temp container releases the injector as well -> no need for "using"
  IInjector injector = tempContainer.CreateInjector();
  ...
}
``` 
As we learned the requested service always must be an interface but the `HttpRequest` implements nothing. That's the time when the [duck typing](https://en.wikipedia.org/wiki/Duck_typing ) comes into view:
```csharp
using Solti.Utils.Proxy;
...
...
using (IServiceContainer tempContainer = Container.CreateChild())
{
  tempContainer.Instance<IHttpRequest>(Request.Act().Like<IHttpRequest>());
  IInjector injector = tempContainer.CreateInjector();
  ...
}
```
...
Notes:
- The [dependency list](https://sholtee.github.io/injector/doc/Solti.Utils.DI.Internals.ServiceReference.html#Solti_Utils_DI_Internals_ServiceReference_Dependencies ) will not contain services requested outside of the constructor.
...
- Starting from version 1.2.0 you can enable *Strict DI* (disabled by default) in `Injector.config.json` which will instruct the injector to throw if a captive dependency can be found.
## MS preferred DI
Microsoft also defines its own [interface](https://docs.microsoft.com/en-us/dotnet/api/system.iserviceprovider?view=netcore-3.1 ) for dependency resolution. Starting from version 1.3.0 this library has built in support for it (see [CreateProvider](https://sholtee.github.io/injector/doc/Solti.Utils.DI.IServiceContainerExtensions.html#Solti_Utils_DI_IServiceContainerExtensions_CreateProvider_Solti_Utils_DI_IServiceContainer_System_IServiceProvider__ ) method).
