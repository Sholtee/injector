...
- **Lazy recipe**: Similar to the service recipe except that the implementation is unknown in build time (e.g. the service implementation is placed in a module we want to load on service request). So instead of using the implementation itself we pass its assembly and type name:
  ...
  - You can use your own type resolver by implementing the `ITypeResolver` interface:
    ```csharp
	// This does the same as the code above. Note that the "LazyTypeResolver" class is a builtin resolver.
...
- **Factory recipe**: As the name suggests services registered by this way have a factory function: 
  ...
  ```
- **Provider recipe**: Providers are services that are responsbile for creating other services.
  ```csharp
  using System;
  using System.Data;
  ...
  public class DbConnectionProvider: IServiceProvider
  {
    ...
    public DbConnectionProvider(IConfiguration config) =>
      config.GetSection($"DbBackends:{Backend}").Bind(Options);
    ...
    public static string Backend {get; set;} = "test"; // or "dev" or "live"
  }
  ...
  container
    .Service<IConfiguration, ...>(Lifetime.Singleton)
    .Provider<IDbConnection, DbConnectionProvider>(Lifetime.Scoped)
    ...
  ```
  Advantages over factory recipe:   
  - Providers have well defined dependencies.
  - They can be configurable (via static members).
  
  Note that this feature was introduced in version 2.0 of the library.
...
- You can setup transactions using proxies as well:
  ...
  ```
- The full version of the above examples can be found [here](https://github.com/Sholtee/injector/blob/master/TEST/UseCases.cs ).
### Auto config
...
- You may request the closed pair of an open generic service without registering it:
  ...
  ```
- To access all services with the given interface you can use the magic `IEnumerable<>` service (starting from version 2.0):
  ```csharp
...
Notes:
- The [dependency list](https://sholtee.github.io/injector/doc/Solti.Utils.DI.ServiceReference.html#Solti_Utils_DI_ServiceReference_Dependencies ) will not contain services requested outside of the constructor.
...
## MS preferred DI
Microsoft also defines its own [interface](https://docs.microsoft.com/en-us/dotnet/api/system.iserviceprovider?view=netcore-3.1 ) for dependency resolution. Starting from version 1.3.0 this library has built in support for it:
```csharp
using System;
...
