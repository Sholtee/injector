...
- Every producible service has its own lifetime, which can be:
  ...
  * `Transient`: Instantiated on every request and released automatically when the parent injector is disposed. Note that the caller may also free the requested service (the system can handle it properly). This is a new behavior since version *1.0.0-preview5*, prior it the caller was responsible for releasing the requested services.
## Workflow of DI
...
- The caller may free the requested service if it is an `IDisposable` and has the `Transient` lifetime. To determine the lifetime call the `injector.LifetimeOf<IMyService>()` method.
...
Sometimes the service being requested is contextual (for instance resolving the `HttpRequest` that triggered the current session). In this case you have two options: deriving a new container for the current session or pass the contextual services to `CreateInjector()` (recommended):
```csharp
using Solti.Utils.DI;
...
// Note that there was no service registered with IHttpRequest interface
using (IInjector injector = Container.CreateInjector((typeof(IHttpRequest), null, Request)))
{
  ...
}
``` 
...
...
using (IInjector injector = Container.CreateInjector((typeof(IHttpRequest), null, Request.Act().Like<IHttpRequest>())))
{
  ...
}
``` 
