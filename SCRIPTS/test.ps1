...
$args="
  ...
  -targetargs:`"test $(Resolve-Path $PROJECT.tests) --framework:netcoreapp2.2 --configuration:Debug --test-adapter-path:. --logger:nunit;LogFilePath=$(Join-Path (Resolve-Path $PROJECT.artifacts) testresults.xml)`"
